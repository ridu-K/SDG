...
def get_location_mapping(data):
    """Create mappings between cities and areas"""
    # Create city to areas mapping
    city_to_areas = {}
    # Create area to city mapping
    area_to_city = {}
    
    for city in sorted(data['City'].unique()):
        areas = sorted(data[data['City'] == city]['Area'].unique())
        city_to_areas[city] = areas
        
        for area in areas:
            area_to_city[area] = city
            
    return city_to_areas, area_to_city
...
def main():
    ...
    city_to_areas, area_to_city = get_location_mapping(data)
    ...
    with tab1:
        ...
        if 'selected_city' not in st.session_state:
            st.session_state.selected_city = list(city_to_areas.keys())[0]
        if 'selected_area' not in st.session_state:
            st.session_state.selected_area = city_to_areas[st.session_state.selected_city][0]
            
        def on_city_change():
            st.session_state.selected_area = city_to_areas[st.session_state.selected_city][0]
            
        def on_area_change():
            st.session_state.selected_city = area_to_city[st.session_state.selected_area]
        ...
        with col2:
            # Area dropdown (filtered based on selected city)
            area = st.selectbox(
                "Select Area",
                options=city_to_areas[city],
                key='selected_area',
    ...
    with tab2:
        ...
        with col1:
            ...
            bathrooms = st.number_input("Number of Bedrooms", 
